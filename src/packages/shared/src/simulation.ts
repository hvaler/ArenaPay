import { ENGINE_VERSION, LEGACY_ENGINE_VERSION, GRID_SIZE, POLICIES, TICKS, inputSchema, replaySchema, seedSchema, nonceSchema, type ArenaState, type Input, type Move, type Player, type Position, type Replay } from './contracts';
import { canonicalJson, sha256Hex } from './hash';
import { runSimulation as runLegacy, seedCommitment as legacyCommitment } from './simulation-v1';
...
export async function verifyReplay(raw: unknown, expectedSeedHash?: string) {
  ...
  const result = runSimulation(replay.seed, replay.inputs, replay.engineVersion);
  const actualHash = await sha256Hex(canonicalJson(result.state));
  const commitment = replay.engineVersion === LEGACY_ENGINE_VERSION ? await legacyCommitment(replay.seed) : await seedCommitment(replay.seed, replay.nonce);
