...
import { apiUrl, publicDemo, siteUrl } from '../lib/runtime';
import type { GameEngineDescriptor } from '../../../../packages/shared/src/game-engine';
...
type GameOption = GameEngineDescriptor & { engineVersion: string };
export function TestnetPanel({ match, onMatch, onSync, onChain, onProof, onRun, running, expanded, shareUrl, engineVersion, onEngineVersion, gameOptions }: { match?: LocalMatch; onMatch: (match: LocalMatch) => void; onSync: (match: LocalMatch) => void; onChain: (chain?: ChainMatch) => void; onProof: (ready: boolean) => void; onRun: () => Promise<void>; running: boolean; expanded: boolean; shareUrl?: string; engineVersion: string; onEngineVersion: (version: string) => void; gameOptions: readonly GameOption[] }) {
  ...
  return <section className="testnet-panel" aria-labelledby="testnet-title">
    <div className="testnet-header"><div><h2 id="testnet-title" tabIndex={-1}>Escrow en Stellar Testnet</h2><p>Inscripción de 1 XLM de prueba por agente. Premio total: 2 XLM de prueba.</p></div><button className="button secondary" disabled={!!busy} onClick={() => void action('Conectando', async () => { const address = await wallet.connect(); setConnected(address); if (!playerA) setPlayerA(address); })}>{connected ? `Freighter: ${short(connected)}` : 'Conectar Freighter'}</button></div>
      {/* Un recorrido se atascó porque Freighter estaba en Mainnet y la cuenta no tenía fondos de
          prueba. Ninguna de las dos cosas se ve desde la página, así que la preparación se explica
          aquí y no solo en la guía. Queda abierta hasta que hay una wallet conectada. */}
      <details className="wallet-help" aria-label="Antes de conectar: prepara Freighter" open={!connected}>
        <summary>Antes de conectar: prepara Freighter</summary>
        <ol>
          <li><strong>Selecciona Testnet.</strong> Abre el selector de red de Freighter, el icono del globo, y cambia de Mainnet a <strong>Testnet</strong>. ArenaPay solo opera en Testnet y rechaza cualquier otra red.</li>
          <li><strong>Consigue XLM de prueba.</strong> Copia tu dirección pública, la que empieza por <code>G</code>, y pégala en <a href="https://lab.stellar.org/account/fund" target="_blank" rel="noreferrer">Stellar Lab · Fund account</a> con la red en Testnet. Friendbot entrega 10.000 XLM ficticios. Repítelo con cada cuenta que vaya a participar.</li>
          <li><strong>Comprueba el saldo.</strong> Vuelve a Freighter, en Testnet, y confirma que aparecen los lumens. El valor en euros o dólares seguirá en cero: los XLM de prueba no valen nada.</li>
        </ol>
        <p>No hace falta la clave secreta, la que empieza por <code>S</code>, ni aquí ni en Stellar Lab. Detalle completo en la <a href={siteUrl('guia-freighter-testnet.html')} target="_blank" rel="noreferrer">guía de preparación</a>.</p>
      </details>
    ...
    {config?.ready && <>
      ...
      {!match?.testnet && expanded && <form className="testnet-create" onSubmit={event => { event.preventDefault(); void action('Creando partida', async () => {
        ...
        const created = await api<LocalMatch>('/matches', { playerA, playerB, buyIn: '10000000', engineVersion });
        onMatch(created); setTransaction(created.testnet?.createTx ?? '');
      }); }}><p>El servidor elige la semilla y la reserva hasta ejecutar la partida financiada. La semilla de práctica no se utiliza aquí.</p><label>Juego<select value={engineVersion} onChange={event => onEngineVersion(event.target.value)} disabled={!!busy}>{gameOptions.map(engine => <option key={engine.engineVersion} value={engine.engineVersion}>{engine.displayName} · {engine.engineVersion}</option>)}</select></label><label>Dirección de Atlas<input value={playerA} onChange={e => setPlayerA(e.target.value.trim())} required placeholder="G…" disabled={!!busy} /></label><label>Dirección de Nova<input value={playerB} onChange={e => setPlayerB(e.target.value.trim())} required placeholder="G…" disabled={!!busy} /></label><button type="submit" className="button primary" disabled={!!busy || !connected}>Crear partida en Testnet</button></form>}
      ...
      {chain && <div className="testnet-state"><div><strong>Estado confirmado: {chain.status}</strong><p>Atlas: {chain.fundedA ? 'depósito confirmado' : 'pendiente'} · Nova: {chain.fundedB ? 'depósito confirmado' : 'pendiente'}</p><p>Última consulta: {updated}. Sincronización cada 5 segundos.</p><p>Vencimiento: ledger {chain.timeoutLedger} · Actual: {snapshot!.ledger}</p>{budget && <p>Tu presupuesto: {xlm(budget.maximum)} XLM · Utilizado: {xlm(budget.spent)} XLM · Vigencia: ledger {budget.expiresLedger}</p>}<details className="budget-details" open><summary>Presupuestos de los participantes</summary>{[['Atlas', snapshot!.budgetA], ['Nova', snapshot!.budgetB]].map(([name, value]) => { const b = value as ChainBudget | null; const available = b ? BigInt(b.maximum) - BigInt(b.spent) : 0n; return <p key={String(name)}><strong>{String(name)}</strong>: {b ? `${xlm(b.spent)} XLM utilizados · ${b.expiresLedger <= snapshot!.ledger ? 'autorización vencida' : `${xlm(String(available > 0n ? available : 0n))} XLM disponibles`}` : 'sin autorización'}</p>; })}</details><p>Solo XLM de prueba en este contrato. Cada depósito requiere firma. Las comisiones no están incluidas; una devolución no restablece el presupuesto gastado.</p></div>
        <div className="testnet-actions"><button className="button secondary" disabled={!!busy} onClick={() => void action('Consultando', refresh)}>Actualizar estado</button>
          ...
          {expired && (isA || isB) && (chain.fundedA || chain.fundedB) && (chain.status === 'Created' || chain.status === 'Funded') && <button className="button secondary" disabled={!!busy} onClick={() => void action('Devolviendo depósitos', () => call('cancel_match', [sc.bytes(match!.testnet!.chainId), sc.address(connected)]))}>Cancelar y devolver depósitos</button>}
