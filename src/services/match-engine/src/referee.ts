...
import { ENGINE_VERSION, LEGACY_ENGINE_VERSION, type Replay } from '../../../packages/shared/src/contracts';
...
export async function signResult(secret: string, contractId: string, localReplay: Replay, chain: ChainMatch, ledger: number): Promise<Resolution> {
  ...
  if (![ENGINE_VERSION, LEGACY_ENGINE_VERSION].includes(chain.engineVersion) || localReplay.engineVersion !== chain.engineVersion) throw new Error('Versión de motor incompatible.');
